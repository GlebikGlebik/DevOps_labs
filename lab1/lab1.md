...
Для начала потребовалось разобраться, что такое nginx. В этом помогло это [видео](https://youtu.be/2aoOEnZmCmQ?si=jeNnbgGmCjcaH3rK), а также [документация](https://nginx.org).
...
Изначально они оба были размещены на локальном сервере и имеют свои доменные имена, что можно увидеть, выполнив в терминале команду:
...
Код для каждого из них, соответственно, выглядел так:


```html
<html><body><h1>My first site</h1></body></html> 
```
...
```html
<html><body><h1>This is website 2</h1><img src='/images/cat.jpg' alt='Котик'>
```
...
Протокол HTTP не обеспечивает безопасности данных, передаваемых между клиентом и сервером. Протокол HTTPS решает эту проблему путём шифрования трафика с использованием сертификатов, поэтому необходимо использовать его и перенаправлять пользователей с HTTP на более безопасный протокол.


Для обеспечения доступа по HTTPS необходимо сгенерировать SSL-сертификаты. Для решения задач данной работы был сгенерирован самоподписанный сертификат с помощью OpenSSL:
